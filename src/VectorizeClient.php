...
use BrynjDigital\LaravelModelVectorize\Exceptions\VectorizeException;
...
class VectorizeClient
{
    ...
    /**
     * Get HTTP client with configured timeouts.
     */
    protected function getHttpClient(): Client
    {
        return new Client([
            'timeout' => config('vectorize.timeout', 30),
            'connect_timeout' => 10,
        ]);
    }
    ...
    public function generateEmbedding(string $text): array
    {
        try {
            ...
            throw new VectorizeException('Invalid response format from Cloudflare Workers AI');
        } catch (VectorizeException $e) {
    ...
    public function insertVectors(array $vectors): array
    {
        try {
            ...
            throw new VectorizeException(
                "Failed to insert vectors: {$e->getMessage()}",
                0,
                $e
            );
        }
    ...
    public function queryVectors(array $queryVector, int $topK = 5, ?array $filter = null): array
    {
        try {
            ...
            throw new VectorizeException(
                "Failed to query vectors: {$e->getMessage()}",
                0,
                $e
            );
        }
    ...
    public function deleteVectors(array $ids): array
    {
        try {
            ...
            throw new VectorizeException(
                "Failed to delete vectors: {$e->getMessage()}",
                0,
                $e
            );
        }
